...
- **ViewBuilder**: builds the derived views from the loaded entities. Will expose separate functions for `build_skills`, `build_experience`, `build_education`, `build_tech_radar`, `build_core_expertise`, `build_featured_projects`, `build_stack_by_domain`, `build_career_story`. Each view is an immutable view model.
